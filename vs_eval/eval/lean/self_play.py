...
def execute_self_play(net):
    ...
    while True:
        ...
        pi = mcts.get_action_probs(board, player, temp=temp)        #   mtcsの盤面の訪問回数配列
        canon = bd.canonical_form(board, player)                    #   canonicalは、playerで掛け算することによって、必ず自分が「1」の盤面になる。
        history.append((canon, pi, player))                         #   historyに1目線盤面+価値データ+自分プレイヤー

        action = int(np.random.choice(len(pi), p=pi))               #   ランダムに選択する。
        board = bd.next_state(board, player, action)                #   actionを選択した盤面を取得

        r = bd.terminal_value(board, player)                        #   ==================================================================
        if r != 0:                                                  #   決着がついた場合の処理
            for canon_b, p, pl in history:
                z = r if pl == player else -r                       #   自分が勝った場合、zに、負けた場合は価値の低い（符号反転)。盤面のプレイヤーは交互になる。
                examples.append((canon_b, p, z))                    #   exampleは、(盤面,プレイヤー,z（価値ですか）
            return examples                                         #   終局までのヒストリーに、全部勝ちをつける。
                                                                    #   ==================================================================
        player = -player
        move_count += 1
...
def train_net(net, examples, optimizer):
    net.train()
    #expamples(盤面,MCTS結果(価値配列),価値(-1/1))をテンソルに
...
def main():
    os.makedirs(CKPT_DIR, exist_ok=True)            #チェックポイントフォルダ作る
    net = PolicyValueNet().to(DEVICE)
    if os.path.exists(CKPT_PATH):                                           #チェックポイントにファイルがあるならそれをロードする
        net.load_state_dict(torch.load(CKPT_PATH, map_location=DEVICE))
        print(f"loaded checkpoint from {CKPT_PATH}")

    optimizer = optim.Adam(net.parameters(), lr=LR)                         #adam最適化

    for it in range(1, N_ITERS + 1):
        t0 = time.time()
        examples = []
        for _ in range(N_SELFPLAY_GAMES):
            examples.extend(execute_self_play(net))                         #これはランダム（現状のNNで）たくさん50対局したときの結果
                                                                            #　(盤面(nanonical)+MCTS結果(訪問回数Array)+価値(1or-1)
                                                                            #　(の全部の打ち手に対する履歴全部)

        train_net(net, examples, optimizer)                                 #ここまでで353回分の手のデータが集まりました。
