...
- 本番学習（`train/self_play.py` の `main()`、N_ITERS=30, N_SELFPLAY_GAMES=50,
  N_SIMS=50、CPU）を実行済み。lossは約2.96（iter 1）→約0.12（iter 30）まで低下し、
  終盤のvs random勝率はW36/D4/L0（40試合中）で安定。
- 学習済み重み `checkpoint/net.pt`（51KB）をリポジトリにコミット済み。
  `.gitignore` は `**/checkpoint/*.pt` を無視しつつ `!checkpoint/net.pt` で
  ルート直下の重みだけ例外的に追跡している。clone後すぐ `python play.py` で
  対戦できる。
...
## 次回やること候補（優先順は特になし）
1. ~~本番学習を実行し、vs randomの勝率・引き分け率の推移を見る~~ → 完了（W36/D4/L0で安定）。
   三目並べは理論上先手完全読みなら相手のミス以外では負けない想定通り、
   引き分け/勝ちのみに収束している。
